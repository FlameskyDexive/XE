...
internal sealed unsafe class VulkanCommandTranslator
{
    ...
    private void EnsureMaterialUniformsBuffer(ShaderBindingSlot[] buffers)
    {
        ...
        for (int i = 0; i < buffers.Length; i++)
        {
            ...
            if (name == "ParticleMaterial")
            {
                Rendering.ParticleMaterialUniformsData data = Rendering.MaterialUniformPacking.PackParticle(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "GradientPS")
            {
                Rendering.GradientSkyboxUniformsData data = Rendering.MaterialUniformPacking.PackGradientSkybox(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "GizmoIconMaterial")
            {
                Rendering.GizmoIconUniformsData data = Rendering.MaterialUniformPacking.PackGizmoIcon(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "TonemapperPS")
            {
                Rendering.TonemapperUniformsData data = Rendering.MaterialUniformPacking.PackTonemapper(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "BloomThresholdPS")
            {
                Rendering.BloomThresholdUniformsData data = Rendering.MaterialUniformPacking.PackBloomThreshold(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "BloomCompositePS")
            {
                Rendering.BloomCompositeUniformsData data = Rendering.MaterialUniformPacking.PackBloomComposite(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "MotionBlurPS")
            {
                Rendering.MotionBlurUniformsData data = Rendering.MaterialUniformPacking.PackMotionBlur(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "AutoExposureAdaptPS")
            {
                Rendering.AutoExposureAdaptUniformsData data = Rendering.MaterialUniformPacking.PackAutoExposureAdapt(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "AutoExposureApplyPS")
            {
                Rendering.AutoExposureApplyUniformsData data = Rendering.MaterialUniformPacking.PackAutoExposureApply(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "GTAOCalculatePS")
            {
                Rendering.GTAOCalculateUniformsData data = Rendering.MaterialUniformPacking.PackGTAOCalculate(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
            ...
            if (name == "GTAOTemporalPS")
            {
                Rendering.GTAOTemporalUniformsData data = Rendering.MaterialUniformPacking.PackGTAOTemporal(_materialProperties, _materialShader);
                ReadOnlySpan<byte> bytes = MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref data, 1));
                _transientUniformBuffers[name] = AllocateTransientUniform(bytes);
                _materialUniformsDirty = false;
                _descriptorDirty = true;
